...
public class OrderServiceImpl {
  ...
  public OrderCreateForm getCreateOrderForm(Long memberId, Long productId) throws Exception {
    ...
    try {
      ...
      OrderMemberDetail orderMemberDetail =
          memberDao.selectAddressAndCouponById(memberId, session).orElseThrow(OrderEntityNotFoundException::new);
  ...
  public OrderCartCreateForm getCreateCartOrderForm(Long memberId) throws Exception {
    ...
    } catch (OrderEntityNotFoundException ex) {
      log.error(ex.getMessage());
      session.rollback();
      throw new EntityNotFoundException(ex.getMessage());
    } catch (Exception ex) {
  ...
  public void cancelOrder(Long orderId) throws Exception {
    ...
    try {
      /* 주문 정보 정보를 취소 상태로 바꿈 */
      Order order = orderDao.selectById(orderId, session).orElseThrow(OrderEntityNotFoundException::new);
      ...
      Delivery delivery = deliveryDao.selectById(orderId, session).orElseThrow(OrderEntityNotFoundException::new);
      ...
      if (isCouponUsed(order.getCouponId())) {
        Coupon coupon =
            couponDao.selectById(order.getCouponId(), session).orElseThrow(OrderEntityNotFoundException::new);
      ...
      Payment payment = paymentDao.selectByOrderId(orderId, session).orElseThrow(OrderEntityNotFoundException::new);
      Member member =
          memberDao.selectById(order.getMemberId(), session).orElseThrow(OrderEntityNotFoundException::new);
