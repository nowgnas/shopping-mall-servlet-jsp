...
public class OrderService {
  ...
  public OrderCreateForm getCreateOrderForm(Long memberId, Long productId) throws Exception {
    ...
    try {
      ProductDetailForOrder productDetail = orderProductManager.determineProductDetailForOrder(productId, session);
      OrderMemberDetail orderMemberDetail = orderMemberManager.determineOrderMemberDetail(memberId, session);
  ...
  public Order createOrder(OrderCreateDto orderCreateDto) throws Exception {
    ...
    try {
      ...
      Product product = orderProductManager.determineProduct(orderCreateDto.getProductId(), session);
      orderProductManager.validateEnoughStockQuantity(product.getQuantity(), orderCreateDto.getQuantity());
      orderProductManager.updateStockQuantity(product, product.getQuantity() - orderCreateDto.getQuantity(), session);
      ...
      orderMemberManager.updateMemberMoney(member, member.getMoney() - orderCreateDto.getTotalPrice(), session);
  ...
  public OrderCartCreateForm getCreateCartOrderForm(Long memberId) throws Exception {
    ...
    try {
      ...
      List<CartAndProductDto> cartAndProductDtos = orderCartManager.determineCartAndProductDtos(memberId, session);
      cartAndProductDtos.forEach(
          cp -> orderProductManager.validateEnoughStockQuantity(cp.getProductQuantity(), cp.getCartProductQuantity()));
  ...
  public Order createCartOrder(OrderCartCreateDto orderCartCreateDto) throws Exception {
    SqlSession session = sessionFactory.openSession();
    try {
      List<CartAndProductDto> cartAndProductDtos = orderCartManager.determineCartAndProductDtos(orderCartCreateDto.getMemberId(), session);
          ...
          .forEach(
              p -> {
                try {
                  ...
                  orderProductManager.validateEnoughStockQuantity(product.getQuantity(), p.getQuantity());
                  orderProductManager.updateStockQuantity(product, product.getQuantity() - p.getQuantity(), session);
      ...
      orderMemberManager.updateMemberMoney(member, member.getMoney() - orderCartCreateDto.getTotalPrice(), session);
  ...
  public void cancelOrder(Long orderId) throws Exception {
    ...
    try {
      ...
      List<ProductOrder> productOrders = orderProductOrderManager.determineProductOrdersByOrderId(orderId, session);
      productOrders.forEach(
          po -> {
            try {
              Product product = orderProductManager.determineProduct(po.getProductId(), session);
              orderProductManager.updateStockQuantity(product, product.getQuantity() + po.getQuantity(), session);
            } catch (Exception e) {
              throw new RuntimeException(e);
            }
          });
      ...
      orderMemberManager.updateMemberMoney(member, member.getMoney() + payment.getActualAmount(), session);
