...
public class CartServiceImpl implements CartService {
  ...
  public AllCartProductInfoDtoWithPagination getCartProductListByMemberPagination(Long memberId)
    ...
    try {
      ...
      Pagination pagination =
          Pagination.builder().currentPage(1).totalPage(Math.toIntExact(totalPaging)).build();

      AllCartProductInfoDtoWithPagination allCartProduct =
  ...
  public AllCartProductInfoDto getCartProductListByMember(Long memberId) throws Exception {
    ...
    try {
      ...
      allProductInfo = productDao.selectProductQuantity(productIdList, session);
  ...
  public void putItemIntoCart(
    ...
    try {
      ...
      if (cartOptional.isPresent()) {
        updateCartService.update(
            cartOptional.get(), requestQuantity, product.getQuantity(), session);
      } else {
        cartDao.insert(
